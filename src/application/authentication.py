...
from src.infrastructure import errors, repositories, security

http_bearer = HTTPBearer(auto_error=False)


class TokensPair(domain.entities.InternalData):
    access_token: str
    refresh_token: str
...
async def get_tokens_pair(username: str, password: str) -> TokensPair:
    ...
    try:
        user = await repositories.User().user_for_auth(username)
